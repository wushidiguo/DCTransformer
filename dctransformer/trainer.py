...
def run(
    ...
    if load_saved and savepath.is_file():
        state_dict = torch.load("./saved/dctransformer.pt")
    ...
    while True:
        ...
        for i, data in enumerate(dataloader):
            ...
            loss = F.cross_entropy(logits_chn.reshape(-1, chn_ncls), data[0][:, 0, :].reshape(-1), ignore_index=0)
            loss += F.cross_entropy(logits_pos.reshape(-1, pos_ncls), data[0][:, 1, :].reshape(-1), ignore_index=0)
            loss += F.cross_entropy(logits_val.reshape(-1, val_ncls), data[0][:, 2, :].reshape(-1), ignore_index=0)
