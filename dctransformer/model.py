...
class Embedding(nn.Module):
    def __init__(self, vocab_size, embed_size=512, dropout=0.1):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embed_size, padding_idx=0)
        self.dropout = dropout
    
    def forward(self, x):
        return F.dropout(self.embedding(x), self.dropout)


class ChunkEmbedding(nn.Module):
    def __init__(self, max_nchunk, embed_size=512, dropout=0.1):
        super().__init__()
        self.embedding = nn.Embedding(max_nchunk, embed_size)
        self.dropout = dropout
...
class ResiduleBlock(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        return x + self.dropout(self.fn(self.layernorm(x), *args, **kwargs))
...
class Encoder(nn.Module):
    def __init__(self, in_channel, downsample, n_layer, dropout, d_model, heads) -> None:
        ...
        for i in range(n_layer):
            self.attns.append(ResiduleBlock(AttentionLayer(d_model, heads), d_model, dropout))
...
class Decoder(nn.Module):
    def __init__(self, n_layer, dropout, d_model, heads, num_cls) -> None:
        ...
        self.ffs = nn.ModuleList()
        for i in range(n_layer):
            self.self_attns.append(ResiduleBlock(AttentionLayer(d_model, heads), d_model, dropout))
            self.cross_attns.append(ResiduleBlock(AttentionLayer(d_model, heads), d_model, dropout))
            self.ffs.append(ResiduleBlock(FeedForward(d_model, dropout), d_model, dropout))
...
class DCTransformer(nn.Module):
    def __init__(self, resolution, block_size, interleave=True, encoder_downsample=2, nlayer=3, d_model=512, nheads=8, chunk_size=896, max_nchunk=10, dropout=0.1) -> None:
        ...
        self.chn_embedding = Embedding(chn_ncls, d_model, dropout)
        self.pos_embedding = Embedding(pos_ncls, d_model, dropout)
        self.val_embedding = Embedding(val_ncls, d_model, dropout)
        self.chunk_embedding = ChunkEmbedding(max_nchunk, d_model, dropout)
    ...
    def decode(self, chunks, priors, chunk_ids, return_digits=True):
        ...
        pos = chunks[:, 1, :]
        val = chunks[:, 2, :]
        # padding_mask = (chn[:, :-1] != 0)[:, None, :, None].to(device)   # (batch_size, 1, seq_len, 1)
        ...
        pos_in_memory[pos_in_memory < 0] = 0    # paddings and eos become zeros here, we will take care of this later
